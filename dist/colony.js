...
(() => {
  ...
  function createPredicate(filter) {
    return function(item) {
      return Object.entries(filter).every(([key, value2]) => item[key] === value2);
    };
  }
  var lastIdByPrefix = {};
  ...
  function mapValues(obj, mapper) {
    return Object.fromEntries(
      Object.entries(obj).map(([key, value2]) => [key, mapper(value2, key)])
    );
  }
  ...
  function Undefined(value2) {
    return value2;
  }
  var BRAND = Symbol("brand");
  function infer(inferable, value2) {
    return isFunction(inferable) ? inferable(value2) : inferable;
  }
    ...
    function createBoxWithGetter(getter, setter2) {
      return function box(setValue2) {
        if (setValue2 === void 0) {
          ...
          ;
          if (isFunction(setValue2)) {
            return box(setValue2(getter()));
          } else {
            setter2(setValue2);
            return setValue2;
          }
  ...
  function isRefs(value2) {
    return value2 && typeof value2 === "object" && values(value2).every((value3) => value3 instanceof Ref);
  }
  ...
  var Ref = class extends Callable {
    constructor(_value) {
      super();
      this._value = _value;
    ...
    map(nestableGetter) {
      ...
      if (isFunction(mapped.value)) {
        ...
        return (...args) => new MappedRef(
          this,
          (value2) => nestableGetter(value2)(...args)
        );
      }
      ;
      return mapped;
    ...
    mapDefined(callback) {
      return this.map((value2) => value2 !== void 0 ? callback(value2) : void 0);
    }
    uses(methods) {
      return assign(this, mapValues(methods, this.map));
    }
    setter(setter = (value2) => this._set(value2)) {
      return new SetterRef(this, setter);
    }
  };
  var readonlyRef = new Ref(5);
  var value = readonlyRef();
  var setValue = readonlyRef(5);
  ...
  var WritableRef = class extends Ref {
    ...
    bridge(forward, backward) {
      return this.map(forward).setter((value2) => this.set(backward(value2)));
    }
    clone() {
  ...
  var SetterRef = class extends WritableRef {
    ...
    set(value2) {
      super.set(value2);
      if (!this.allowMismatch && this.value !== value2) {
  ...
  function tag(tagName) {
    ...
    function verboseFactory(props, children) {
      ...
      props && forEach(
        props,
        (value2, key) => {
          typeof value2 === "function" ? element[key] = value2 : $with(value2, (refable) => {
            ...
            function update(value3) {
              typeof value3 === "boolean" ? value3 ? element.setAttribute(key, "") : element.removeAttribute(key) : element.setAttribute(key, String(value3));
            }
            ;
          });
        }
  ...
  function If(condition, ifYes, ifNo = Undefined()) {
    return condition.map((value2) => value2 ? infer(ifYes, value2) : infer(ifNo));
  }
