(() => {
  ...
  function render_compiled() {
    ...
    (() => {
      ...
      function isoStringToTimestamp2(isoString) {
        return isoString ? new Date(isoString).getTime() : 0;
      }
      function sortByDate2(items, dateAccessor = (item) => item.created_at) {
        return items.sort((a2, b) => isoStringToTimestamp2(dateAccessor(a2)) - isoStringToTimestamp2(dateAccessor(b)));
      }
      function renameKeys(record, keyMap) {
        return mapKeys2(record, (key) => keyMap[key] ?? key);
      }
      var SUPPORTED_TAGS = [
        "html",
        "head",
            ...
            div({
              ...
              div({
                ...
                div({
                  ...
                  button({}, { onclick() {
                    const html2 = `<script>window.colonyData=${JSON.stringify({ graphData: rawData, in3D: in3D2 })}<\/script><script>(${"render_compiled" in window && typeof window.render_compiled === "function" ? window.render_compiled.toString() : $throw2("render_compiled not found")})()<\/script>`;
